...
x_test = x_train * 2    # test데이터는 train데이터의 2배로 만들었다.
y_test = y_train * 2
...
# 모델 구성하기
model = Sequential()
model.add(LSTM(128, batch_input_shape=(1, 4, 1), stateful=True))    #상태유지이므로 True 
...
for epoch_idx in range(num_epochs):
    ...
    model.reset_states()    # 다음 훈련할 데이터가 이전의 훈련할 데이터와 완전히 연관이 없을 때 사용하는 것.
