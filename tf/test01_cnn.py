...
print('[1] X_img : ', X_img)    # (?, 28, 28, 1)
# L1 ImgIn shape=(?, 28, 28, 1)
W1 = tf.Variable(tf.random_normal([3, 3, 1, 32], stddev=0.01))
#    Conv     -> (?, 28, 28, 32)
#    Pool     -> (?, 14, 14, 32)
print('[2] W1 : ', W1)          # (3, 3, 1, 32)
L1 = tf.nn.conv2d(X_img, W1, strides=[1, 1, 1, 1], padding='SAME')
print('[3] L1 : ', L1)          # (?, 28, 28, 32)
L1 = tf.nn.relu(L1)
print('[4] L1 : ', L1)          # (?, 28, 28, 32)
# L1 = tf.nn.max_pool(L1, ksize=[1, 2, 2, 1],
#                     strides=[1, 2, 2, 1], padding='SAME')
L1 = tf.nn.max_pool(L1, ksize=[1, 2, 2, 1],
                      strides=[1, 2, 2, 1], padding='VALID') 
print('[5] L1 : ', L1)          # 1221, 1221, SAME  : (?, 14, 14, 32)
                                # 1221, 1221, VALID : (?, 14, 14, 32)
                                # 1221, 1111, VALID : (?, 27, 27, 32)
                                # 1221, 1111, SAME  : (?, 28, 28, 32)
                                # 1111, 1111, VALID : (?, 27, 27, 32)
                                # 1111, 1111, SAME  : (?, 28, 28, 32)
